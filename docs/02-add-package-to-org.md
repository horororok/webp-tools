...
[<new-tool> @ emsdk=3.1.74 libwebp=1.6.0 platform=linux/amd64 env=web,worker]
...
## 5. QA — playground에 섹션 추가

도구는 `ENVIRONMENT=web,worker`라 Node 실행이 안 되므로 **브라우저 playground로
수동 QA** (Node 스모크 X — 왜 그런지는 `01-create-npm-library.md` §5 참고).

기존 `examples/playground`에 새 도구용 섹션/탭을 하나 추가:
- `examples/playground/package.json`의 `dependencies`에 `@btheegg-kimth/<new-tool>: workspace:*`
- `src/main.ts`에 새 도구 import + 변환 버튼/미리보기 하나 더
- 입력 형식에 맞는 최소 fixture를 base64로 임베드 (GIF는 직접 인코딩 가능, PNG/JPEG는
  작은 fixture commit)
...
브라우저에서 새 도구 변환이 결과를 띄우고 매직이 맞으면 통과.
...
- playground에서 실제 변환이 브라우저에서 동작 + magic 검증
- (소비자 경로까지) `npm pack` → 별도 프로젝트 설치해 `vite build` 경고 없는지
...
[ ] examples/playground에 새 도구 섹션 추가
[ ] pnpm install && pnpm build:wasm && pnpm -r build && pnpm qa (브라우저 확인)
[ ] 빌드 산출물에 pthread/Worker/import("module") 패턴 없는지 확인 (부록 B 참고)
...
| 항목 | 값 | 이유 |
|---|---|---|
| `ENVIRONMENT` | `web,worker` | **node 넣지 말 것.** node를 넣으면 `import("module")`이 박혀 소비자 Vite 빌드에서 "Module externalized" 경고 + exclude 강요. worker는 남겨 소비자가 Web Worker에서 변환 가능 |
...
| `SINGLE_FILE` | `1` | wasm을 mjs에 base64 인라인 → 번들러 친화 (`.wasm` 별도 호스팅 X) |
| `MODULARIZE` + `EXPORT_ES6` | `1` | `.mjs` ES 모듈 형태 |
| libwebp `WEBP_USE_THREAD` | `OFF` | **필수.** ON이면 COOP/COEP + SharedArrayBuffer 요구 → 일반 환경에서 못 씀 |
